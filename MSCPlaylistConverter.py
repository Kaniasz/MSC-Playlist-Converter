...
def confirm_and_clean_radio_folder(master, folder):
    ...
    if existing_files:
        proceed = messagebox.askyesno(
            ...
            "The folder is not empty and contains these files:\n\n" +
            "\n".join(existing_files) +
            "\n\nDo you want to DELETE ALL FILES in this folder and continue?"
        )
        if not proceed:
            return False
...
class MSCPlaylistGUI:
    def __init__(self, master):
        ...
        try:
            ...
            pass
        tk.Button(row1, text="Convert Folder", command=self.convert_folder).pack(side="left", padx=(8, 0))
        ...
        self.open_output_btn = tk.Button(mode_frame, text="Output Folder", command=self.open_output_folder)
        self.open_output_btn.pack(side="left", padx=(22,0))
        ...
        self.open_output_btn.bind("<Enter>", self.show_output_folder_tooltip)
        self.open_output_btn.bind("<Leave>", self.hide_output_folder_tooltip)

        self.log_frame = tk.Frame(main_frame)
        self.log_shown = False
        self.master.bind('<F7>', self.toggle_log)
        self.log_label = tk.Label(self.log_frame, text="Log:")
        self.log_text = ScrolledText(self.log_frame, height=4, width=62, state="disabled", font=("TkDefaultFont", 8))
        self.copy_log_btn = tk.Button(self.log_frame, text="Copy Log", command=self.copy_log)
    ...
    def show_log(self):
        if not self.log_shown:
            self.master.geometry(self.large_geometry)
            self.log_label.pack(anchor="w")
            self.log_text.pack(fill="x")
            self.copy_log_btn.pack(anchor="e", pady=(0,4))
    ...
    def hide_log(self):
        if self.log_shown:
            ...
            self.copy_log_btn.pack_forget()
            self.log_frame.pack_forget()
            self.log_shown = False
            self.master.geometry(self.small_geometry)
    ...
    def copy_log(self):
        ...
        self.master.clipboard_append(self.log_text.get("1.0", "end").strip())
    ...
    def import_coverart(self):
        ...
        self.log(f"Imported cover image: {os.path.basename(path)}")
    ...
    def safe_after(self, func, *args, **kwargs):
        if self.cancel_flag.is_set():
            return
        self.master.after(0, func, *args, **kwargs)
    ...
    def show_success(self, files):
        ...
        self.log(f"Success! Saved {len(files)} file(s) to: {self.get_output_folder()}")
    ...
    def start_download(self):
        ...
        if total == 0:
            self.show_error("No songs to process. Please enter a link or use the Convert Folder button.")
            return
        ...
        def task():
            ...
            try:
                ...
                for i, media_url in enumerate(urls_to_dl, 1):
                    ...
                    if is_cd and not coverart_done and not coverart_path and thumbnail:
                        ...
                        try:
                            ...
                            coverart_done = True
                            self.log("Fetched cover art from first track thumbnail.")
                        except Exception:
                            pass
                ...
                if is_cd:
                    ...
                    if coverart_path:
                        try:
                            ...
                            self.log(f"CD cover saved as {coverart_final}")
                        except Exception as e:
                            self.log(f"Failed to set coverart: {str(e)}")
                    else:
                        img = Image.new("RGB", (512,512), (30,30,30))
                        img.save(coverart_final)
                        self.safe_after(self.update_coverart_thumbnail, coverart_final)
                        self.log("Blank CD cover created.")
                ...
                self.safe_after(self.cancel_button.config, state='disabled')
                self.safe_after(self.log, f"[ERROR] {str(e)}")
            finally:
                self.safe_after(self.download_button.config, state='normal')
    ...
    def convert_folder(self):
        ...
        self.cancel_button.config(state='normal')
        self.cancel_flag.clear()
        self.log("Starting conversion from folder ...")
    ...
    def cancel_download(self):
        ...
        if self.current_thread and self.current_thread.is_alive():
            # The thread should exit promptly due to cancel_flag; join for 1s for safety
